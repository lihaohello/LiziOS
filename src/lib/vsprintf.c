...
static char* number(char* str,
    ...
    // 如果 flags 指出是带符号数并且数值 num 小于 0，则置符号变量
    // sign=负号，并使 num 取绝对值
    if (flags & DOUBLE && (*(double*)(num)) < 0) {
        ...
        *(double*)(num) = -(*(double*)(num));
        ...
        // 否则如果 flags 指出是加号，则置 sign=加号，否则若类型带空格标志则
        // sign=空格，否则置 0
    ...
    // 若 flags 中没有左调整（左对齐）标志, 则在剩余宽度中存放 c
    // 字符（'0'或空格）
    if (!(flags & LEFT))
        while (size-- > 0)
            *str++ = pad;
    ...
    repeat:
        ...
        switch (*fmt) {
            ...
            case 'f':
                ...
                str =
                    number(str, (u32*)&dnum, 10, field_width, precision, flags);
                break;
            case 'b':  // binary
                num = va_arg(args, unsigned long);
